...
class ResNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.avgpool(x)
        second_last_outputs = torch.flatten(x, 1)
        x = self.fc(second_last_outputs)
...
class MResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        second_last_outputs = out.view(out.size(0), -1) # out.size(0) = out.reshape(0)
        out = self.fc(second_last_outputs)
        
        return out, second_last_outputs
