...
class ArithmeticDataset:
    ...
    def splits(
        ...
        train_rows, actual_train_rows, _ = cls.calc_split_len(train_pct, actual_train_pct, len(eqs)) # return 3 paramters here

        train_ds = cls(ds_name, eqs[:actual_train_rows], train=True, data_dir=data_dir)
        val_ds = cls(ds_name, eqs[train_rows:], train=False, data_dir=data_dir)

        return train_ds, val_ds

    @classmethod
    def calc_split_len(cls, train_pct, actual_train_pct, ds_len): # modify here to pass into the argument.
        train_rows = round(ds_len * (train_pct / 100.0))
        val_rows = ds_len - train_rows
        actual_train_rows = round(ds_len*(actual_train_pct / 100.0))
        return train_rows, actual_train_rows, val_rows
