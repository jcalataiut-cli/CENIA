# bert_sst_sgd_fp32_eval_each_epoch.py
...
def train_one_epoch(model, dataloader, optimizer, device, args):
    ...
    for batch in tqdm(dataloader, desc="Train", leave=False, disable=True):
        ...
        second_last_outputs = outputs.hidden_states[-1][:, 0, :]  # CLS
        ...
        if args.weight_cap:
            with torch.no_grad():
                if args.cap_type == "spectral":
                    _ = spectral_cap(model.classifier.weight, s_max=5.0, n_iter=1)
                elif args.cap_type == "frob":
                    _ = frobenius_cap(model.classifier.weight, args.f_max_norm)
...
def main():
    ...
    ap.add_argument("--task", type=str, default="sst2", choices=["sst2", "sst5"])
    # ap.add_argument("--model_name", type=str, default="huawei-noah/TinyBERT_General_4L_312D")
    ap.add_argument("--model_name", type=str, default="bert-base-uncased")
    ap.add_argument("--batch_size", type=int, default=64)
    ap.add_argument("--epochs", type=int, default=8)
    ap.add_argument("--lr", type=float, default=6e-4)
    ap.add_argument("--momentum", type=float, default=0.9)
    ap.add_argument("--dropout", type=float, default=0.0, help="BERT hidden/attn dropout")
    ...
    args.output_dir = "BERT_{}_Seed{}_Date{}_EPOCH{}_CANCEL{}_BUMP{}_BSZ{}_TSZ{}_LR{}_HD{}_DP{}_WD{}_REGR{}_PHIN{}_LAMBDA{}_TAU{}_WCAP{}_CAPT{}_FMAX{}".format(
        args.task.upper(),
        args.seed,
        args.training_date,
        args.epochs,
    ...
    model = BertForSequenceClassification.from_pretrained(args.model_name, config=config).to(device)
    ...
    print("Preparation for regularizer cancellation:")
    print(f"  BASE_LR      = {BASE_LR}")
    print(f"  CANCEL_EPOCH = {CANCEL_EPOCH}")
    print(f"  WD_AFTER     = {WD_AFTER}")
    print(f"  BUMP_X       = {BUMP_X}")
