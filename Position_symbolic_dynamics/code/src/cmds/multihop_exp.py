...
from ..data.dataset import (
    SequenceMultiHopTokenizer,
    SequenceMultiHopDatasetGenerator,
    SequenceMultiHopCollator,
)
...
def main(config: ExperimentConfig):
    ...
    # If there is no explicit vocab_size_numbers, it will be the minimum required
    # for the task: i.e., context_en
    vocab_size_numbers = (
        config.vocab_size_numbers if config.vocab_size_numbers else config.context_len
    )
    tokenizer = SequenceMultiHopTokenizer(
        vocab_size_numbers=config.context_len, vocab_size_str=config.vocab_size
    )
    collator = SequenceMultiHopCollator(tokenizer)
    ...
    gpt_config = GPTJConfig(
        ...
        resid_pdrop=config.resid_pdrop,  # Dropout rate 0.1
        attn_pdrop=config.attn_pdrop,  # Dropout rate 0.1
        embed_pdrop=config.embed_pdrop,  # Dropout rate 0.1
    ...
    training_args = TrainingArguments(
        ...
        adam_beta1=config.adam_beta1,  # From paper
        adam_beta2=config.adam_beta2,  # From paper
        # --- Scheduler (Linear w/ Warmup) ---
        lr_scheduler_type="linear",
        warmup_steps=750,  # From paper
        # --- Regularization ---
        weight_decay=config.weight_decay,  # From paper
        # --- Logging and Saving ---
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Multihop experiment")
